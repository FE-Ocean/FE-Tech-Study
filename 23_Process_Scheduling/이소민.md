...
- 프로세스 스케줄링은 운영체제의 특징과 시스템 요구사항에 따라 다양한 알고리즘 방식으로 동작. 
<br/>
...
- 비선점형과 선점형 존재
- 평균 대기 시간을 줄일 수 있음
- 다음 프로세스의 CPU burst time 예측 어려움
- 프로세스의 실행 시간(버스트 시간)이 길면 계속해서 후순위가 됨 - 기아문제 발생
...
- 기아문제 해결 위해 aging(시간 지날수록 프로세스 우선순위 높이는 방식) 사용 가능
...
- 시간 할당량이 너무 작으면 context switching에 따른 오버헤드가 크게 증가함
...
- single queue에서는 모든 프로세서의 우선순위를 찾아야 하기 때문에 시간 복잡도가 Q(n)
...
### 참고자료 
https://namu.wiki/w/%ED%94%84%EB%A1%9C%EC%84%B8%EC%8A%A4%20%EC%8A%A4%EC%BC%80%EC%A4%84%EB%A7%81#s-3.1<br/>
...
https://ko.wikipedia.org/wiki/%EB%9D%BC%EC%9A%B4%EB%93%9C_%EB%A1%9C%EB%B9%88_%EC%8A%A4%EC%BC%80%EC%A4%84%EB%A7%81<br/>
...
https://wpaud16.tistory.com/entry/%EC%9A%B4%EC%98%81%EC%B2%B4%EC%A0%9C-Multilevel-Feedback-Queue-%EB%A9%80%ED%8B%B0%EB%A0%88%EB%B2%A8-%ED%94%BC%EB%93%9C%EB%B0%B1-%ED%81%90?category=954432<br/>